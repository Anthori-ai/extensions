(function () {
  ...
  function describeRuntimeError(error, fallbackMessage, options = {}) {
    const providerLabel = String(options.providerLabel || "OpenAI").trim() || "OpenAI"
    ...
    if (
      ...
      return {
        message: `${providerLabel} did not respond in time. Check your network connection and provider settings.`,
    ...
    if (
      ...
      return {
        message: `Could not connect to ${providerLabel}. Check your network connection and provider settings.`,
        detail: normalizedDetail,
      }
    }
    if (
      ...
      return {
        message: `Could not resolve the ${providerLabel} server address. Check your network connection and provider settings.`,
        detail: normalizedDetail,
      }
    }
    if (
      ...
      return {
        message: `${providerLabel} rejected the request. Check your credentials.`,
        detail: normalizedDetail,
      }
    }
    return {
      message: normalizedDetail || String(fallbackMessage || "Request failed.").trim(),
      detail: normalizedDetail,
    }
  }
  ...
  function init() {
    ...
    const root = document.getElementById("openai-config-root")
    ...
    function hasApiKeyAuthorization() {
    ...
    function renderModelOptions(items, selectedValue, placeholderText) {
      ...
      placeholder.value = ""
      placeholder.textContent = String(placeholderText || "(optional)")
    ...
    async function refreshModels() {
      ...
      try {
        ...
        if (!hasApiKeyAuthorization()) {
          renderModelOptions([], selectedValue, "(enter an API key)")
          setInlineStatus(statusEl, "", "")
          return
        }
        ...
        renderModelOptions(items, selectedValue, "(optional)")
        if (!Array.isArray(items) || items.length === 0) {
          setInlineStatus(statusEl, "No models were returned.", "warning")
        } else {
          setInlineStatus(statusEl, "", "")
        }
      } catch (error) {
        renderModelOptions([], selectedValue, hasApiKeyAuthorization() ? "(optional)" : "(enter an API key)")
        const runtimeError = describeRuntimeError(error, "Failed to load OpenAI models.")
    ...
    async function load() {
      ...
      try {
        ...
        const runtimeError = describeRuntimeError(error, "Failed to load the OpenAI provider UI.")
        setInlineStatus(statusEl, runtimeError.message, "error", { detail: runtimeError.detail })
      } finally {
        setUiReady(true)
      }
